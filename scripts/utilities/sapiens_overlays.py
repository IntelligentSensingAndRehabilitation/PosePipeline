...
Usage:
    python sapiens_overlays.py <filename> [options]
...
def visualize_seg(frame, seg_mask, present, bbox, img_size, alpha=0.6):
    ...
    colored_mask = GOLIATH_PALETTE[seg_mask]
...
def find_video_key(filename, project=None, tracking_method=21, video_subject_id=0):
    """Find a unique video key, optionally auto-detecting project."""
    base_restriction = {
    ...
    if not filename.endswith('.mp4'):
        # Try both with and without .mp4
        filename_variants = [filename, f"{filename}.mp4"]
    else:
        filename_variants = [filename, filename[:-4]]

    for fn in filename_variants:
        restriction = {**base_restriction, 'filename': fn}

        if project:
            restriction['video_project'] = project
            keys = (PersonBbox & restriction).fetch('KEY')
            if len(keys) == 1:
                return keys[0]
            elif len(keys) > 1:
                raise ValueError(f"Multiple matches found for {fn} in project {project}. Please specify video_subject_id.")
            # If no match with project, continue to try without
        else:
            # Auto-detect project
            keys = (PersonBbox & restriction).fetch('KEY')
            if len(keys) == 1:
                ...
                projects = list(set(k['video_project'] for k in keys))
                raise ValueError(
                    f"Multiple projects found for '{fn}': {projects}. "
                    f"Please specify --project to disambiguate."
                )
...
def run_all_modes(filename, project=None, variant="0.3b", tracking_method=21, video_subject_id=0):
    ...
    base_filename = key['filename'].replace('.mp4', '')
    output_path = f"{base_filename}_sapiens_{variant}_all_modes.mp4"
...
def main():
    parser = argparse.ArgumentParser(
        description="Run all four Sapiens modes and create 2x2 tiled visualization",
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog=__doc__
    )

    parser.add_argument("filename", help="Video filename (with or without .mp4 extension)")
    parser.add_argument("--project", "-p", help="Video project name (auto-detected if unique)")
    parser.add_argument(
        "--variant", "-v",
        default="0.3b",
        choices=["0.3b", "0.6b", "1b", "2b"],
        help="Sapiens model variant (default: 0.3b)"
    )
    parser.add_argument(
        "--tracking-method", "-t",
        type=int,
        default=21,
        help="Tracking method ID (default: 21)"
    )
    parser.add_argument(
        "--subject-id", "-s",
        type=int,
